...
module.exports = function propertySteps() {
     ...
     */
     ...
     this.Given(/^I add file (.+) to request body$/, function step(relPath, done) {
        this._log.info('I add file %s to request body', relPath);
        var req = this.req;
        var testDataRoot = this.testConfig.testDataRoot || process.cwd();
        var fullPath = path.join(testDataRoot, relPath);
        delete req.body;
        req.formData = {
