...
module.exports = function propertySteps() {
    ...
    function checkProperty(propertyString, check, valueString) {
        ...
        // compare checksums if we have a file and the response is a file too.
        if (Buffer.isBuffer(actual) && isStream(expected)) {
            // console.log(expected);
            var path = expected.path;
            this._log.trace({ path: path }, 'Loading file for checksum');
            var fileContents = fs.readFileSync(path).toString();
            actual = checksum(actual.toString());
            expected = checksum(fileContents);
        }
