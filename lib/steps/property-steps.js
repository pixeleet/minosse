...
module.exports = function propertySteps() {
    ...
    function compare(actual, check, expected) {
        if ((Buffer.isBuffer(actual) && isStream(expected)) ||
            (isStream(actual) && Buffer.isBuffer(expected)) ||
            (Buffer.isBuffer(actual) && Buffer.isBuffer(expected)) ||
            (isStream(actual) && isStream(expected))) {

            actual = Buffer.isBuffer(actual) ? checksum(actual) : checksum(fs.readFileSync(actual.path));
            expected = Buffer.isBuffer(expected) ? checksum(expected) : checksum(fs.readFileSync(expected.path));
        }
