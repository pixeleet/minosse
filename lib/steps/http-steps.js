...
module.exports = function httpSteps() {
    ...
    function makeRequest(method, path, callback) {
        this._log.info('Step: send a %s request to %s', method, path);
        var _this = this;
     ...
     */
    this.Then(/^I send a (GET|PUT|POST|DELETE|HEAD|OPTS|PATCH) request to (\S+)$/, makeRequest);

    /* eslint-disable max-len */
    this.Then(/^I poll with a (GET|PUT|POST|DELETE|HEAD|OPTS|PATCH) request to (\S+) until (?:property|the) (.+?) (equals|does not equals) (.+)$/,
    /* eslint-enable */
            function (httpMethod, path, propertyString, check, valueString, done) {
