...
var loadFileData = require('../parsers').file;
...
module.exports = function httpSteps() {
     ...
     */
    this.Given(/^I secure the connection with certificate (\S+) and with key (\S+)$/,
            function addHeader(certFilePath, keyFilePath, done) {
        this._log.info('Step: I secure the connection with certificate %s and with key %s', certFilePath, keyFilePath);
        this.req.agentOptions = {
            cert: loadFileData.call(this, certFilePath),
            key: loadFileData.call(this, keyFilePath)
        };
        done();
    });
