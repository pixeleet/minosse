...
module.exports = function httpSteps() {
    ...
    this.Given(/^I set content type to (application\/json|multipart\/formdata)$/,
            function setRequestType(contentType, done) {
        this._log.info('Step: I set content type to %s', contentType);
    ...
    function makeRequest(method, path, callback) {
        ...
        var type = this.req.type;
        var headers = this.req.headers;

        if (!type || type === JSON_TYPE || headers['content-type'] === JSON_TYPE) {
            this.req.json = true;
        } else if (type === FORM_TYPE || headers['content-type'] === FORM_TYPE) {
            var file = this.req.body.file;
            this.req.formData = file;
